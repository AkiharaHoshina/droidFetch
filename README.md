...
一款专为 Android 环境深度定制的系统信息命令行展示工具。无论你是在使用 Termux、ADB Shell，还是配置了 Chroot/PRoot 的 Linux-on-Android 环境，`droidFetch` 都能像 `neofetch` 一样，在终端中优雅地展示你的设备软硬件信息。

## ✨ 核心特性

* **深度 ROM/发行版识别:** 通过读取系统属性（`getprop`），精准识别主流 Android 定制系统及第三方 ROM。
* **精美的 ASCII 图标:** 内置超过 15 种 Android 系统的专属 ASCII 艺术徽标与主题配色。
